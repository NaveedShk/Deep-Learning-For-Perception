...
def rnn_step_backward(dnext_h, cache):
    ...
    # Gradient through tanh: d(tanh)/dz = 1 - tanh^2
...
def rnn_backward(dh, cache):
    ...
    # Replace "pass" statement with your code
    N, T, H = dh.shape
    _, D = cache[0][0].shape  # x shape from first step cache
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        features = self.image_encoder(images)  # (N, C, 4, 4)
        N, C, h_feat, w_feat = features.shape
        if self.cell_type in ("rnn", "lstm"):
            # Global average pool -> project to h0
            pooled = features.mean(dim=(2, 3))  # (N, C)
            h0 = self.feature_proj(pooled)       # (N, H)
            word_vecs = self.embedding(captions_in)  # (N, T, W)
            hidden = self.rnn(word_vecs, h0)         # (N, T, H)
        else:  # attn
            # Project each spatial location: (N, C, 4, 4) -> (N, H, 4, 4)
            A = self.feature_proj(features.permute(0, 2, 3, 1).reshape(N, -1, C))
            A = A.reshape(N, h_feat, w_feat, -1).permute(0, 3, 1, 2)  # (N, H, 4, 4)
            word_vecs = self.embedding(captions_in)  # (N, T, W)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type in ("rnn", "lstm"):
            ...
            h = self.feature_proj(pooled)  # (N, H)
        ...
        word = self._start * images.new(N).fill_(1).long()  # (N,)

        for t in range(max_length):
            word_vec = self.embedding(word.unsqueeze(1)).squeeze(1)  # (N, W)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        gates = x @ self.Wx + prev_h @ self.Wh + self.b  # (N, 4H)
        H = prev_h.shape[1]
        i = torch.sigmoid(gates[:, :H])
        f = torch.sigmoid(gates[:, H:2*H])
        o = torch.sigmoid(gates[:, 2*H:3*H])
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # A: (N, H, 4, 4), prev_h: (N, H)
    # Flatten spatial dims: A_flat -> (N, H, 16)
    A_flat = A.reshape(N, H, -1)           # (N, H, 16)
    # Scaled dot product: prev_h (N, H) x A_flat (N, H, 16) -> (N, 16)
    scale = math.sqrt(H)
    scores = torch.bmm(prev_h.unsqueeze(1), A_flat).squeeze(1) / scale  # (N, 16)
    attn_weights = F.softmax(scores, dim=1).reshape(N, D_a, D_a)        # (N, 4, 4)
    # Weighted sum of spatial features
    attn = torch.bmm(A_flat, attn_weights.reshape(N, -1, 1)).squeeze(2)  # (N, H)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        next_h, next_c = None, None
        # Replace "pass" statement with your code
        gates = x @ self.Wx + prev_h @ self.Wh + attn @ self.Wattn + self.b  # (N, 4H)
        H = prev_h.shape[1]
        i = torch.sigmoid(gates[:, :H])
        f = torch.sigmoid(gates[:, H:2*H])
        o = torch.sigmoid(gates[:, 2*H:3*H])
