...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        # Compute scaled dot product weights for query i
        ...
        out[i] = torch.matmul(weights_softmax, value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        # Compute attention weights for batch element n
        weights = torch.zeros(K, K)
        for i in range(K):
            for j in range(K):
                weights[i, j] = torch.dot(query[n, i], key[n, j]) / (M ** 0.5)
        
        # Apply softmax across keys for each query
        weights_softmax = F.softmax(weights, dim=1)
        
        # Compute weighted sum of values using matrix multiplication
        out[n] = torch.matmul(weights_softmax, value[n])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    # Compute scaled dot product: (N, K, M) @ (N, M, K) -> (N, K, K)
    ...
    # Compute weighted sum: (N, K, K) @ (N, K, M) -> (N, K, M)
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        k_transformed = self.k(key)
        v_transformed = self.v(value)
        
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(
            q_transformed, k_transformed, v_transformed, mask
        )
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.attention = MultiHeadAttention(num_heads, emb_dim, dim_out)
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    # Create upper triangular matrix (exclude diagonal) - these positions should be masked
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        # Replace "pass" statement with your code
        dim_out = emb_dim // num_heads
        self.attention_self = MultiHeadAttention(num_heads, emb_dim, dim_out)
        self.attention_cross = MultiHeadAttention(num_heads, emb_dim, dim_out)
        self.feed_forward = FeedForwardBlock(emb_dim, feedforward_dim)
        self.norm1 = LayerNormalization(emb_dim)
        self.norm2 = LayerNormalization(emb_dim)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    # Create position vector: [0/K, 1/K, 2/K, ..., (K-1)/K]
    positions = torch.arange(K, dtype=torch.float32) / K
    # Replicate M times and reshape to (1, K, M)
    y = positions.unsqueeze(0).unsqueeze(-1).repeat(1, 1, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    position = torch.arange(K, dtype=torch.float32).unsqueeze(1)  # (K, 1)
    
    # Create dimension indices
    div_term = torch.exp(torch.arange(0, M, 2, dtype=torch.float32) * (-torch.log(torch.tensor(10000.0)) / M))
    
    # Initialize output tensor
    y = torch.zeros(1, K, M)
    
    # Apply sine to even indices
    y[0, :, 0::2] = torch.sin(position * div_term)
    
    # Apply cosine to odd indices
    y[0, :, 1::2] = torch.cos(position * div_term)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        # Create mask for decoder (based on answer sequence without last token)
        mask = get_subsequent_mask(ans_b[:, :-1])
        
        # Pass through decoder
        dec_out = self.decoder(a_emb_inp, enc_out, mask)
        
        # Reshape to (N*O, V) where O is output sequence length
        N, O, V = dec_out.shape
        dec_out = dec_out.contiguous().view(N * O, V)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
